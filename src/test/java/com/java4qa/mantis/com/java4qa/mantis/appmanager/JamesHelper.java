...
//
//    // Don't know why it doesn't allow login at the firast attemp
//    readUntil("Login id:");
//    write(login);
//    readUntil("Password:");
//    write(password);
...
//    readUntil("Welcome " + login + ". HELP for a list of commands");
  ...
  private List<MailMessage> getAllMail(String username, String password) throws MessagingException {
    Folder inbox = openInbox(username, password);
    List<MailMessage> message = Arrays.asList(inbox.getMessages()).stream().map( (m) -> toModelMail(m)).collect(Collectors.toList());
    closeFolder(inbox);
    return message;
  }
